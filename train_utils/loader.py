    ...
    def load(self) :
        ...
        for i, dataset_name in enumerate(self.datasets) :
            ...
            # This dataset dataset consists of 7,787 genuine grade-school level, multiple-choice science questions, assembled to encourage research in advanced question-answering.
                ...
                dataset = challenge_dataset.shuffle(self.random_seed)

            elif "arc-e" in dataset_name :
                dataset_path = "ai2_arc"
                if self.cache_dir is not None :
                ...
                dataset = easy_dataset.shuffle(self.random_seed)
            ...
            # This dataset's goal is to choose the right option for a given sentence which requires commonsense reasoning. (Formulated as a fill-in-a-blank task with binary options)
                ...
                if self.cache_dir is not None :
                    dataset = load_dataset(dataset_path, dataset_name, cache_dir=self.cache_dir, trust_remote_code=True)
                else :
            ...
            elif "siqa" in dataset_name :
                dataset_path = "lighteval/siqa"
                if self.cache_dir is not None :
                    dataset = load_dataset(dataset_path, "default", cache_dir=self.cache_dir, trust_remote_code=True)
                else :
                    dataset = load_dataset(dataset_path, "default", trust_remote_code=True)
                ...
                if self.cache_dir is not None :
                    dataset = load_dataset(dataset_path, 'plain_text', cache_dir=self.cache_dir, trust_remote_code=True)
                else :
                    dataset = load_dataset(dataset_path, 'plain_text', trust_remote_code=True)

                dataset = dataset["train"]
                dataset = dataset.shuffle(self.random_seed)

            elif "obqa" in dataset_name :
                dataset_path = "allenai/openbookqa"

                if self.cache_dir is not None :
                    dataset = load_dataset(dataset_path, 'main', cache_dir=self.cache_dir, trust_remote_code=True)
                else :
                    dataset = load_dataset(dataset_path, 'main', trust_remote_code=True)
        ...
        for i, dataset_name in enumerate(self.datasets) : 
            if "arc-c" in dataset_name :
                ...
                if self.cache_dir is not None :
                    ...
                    challenge_dataset = load_dataset(dataset_path, "ARC-Challenge")
                dataset = challenge_dataset["test"]
            
            elif "arc-e" in dataset_name :
                dataset_path = "ai2_arc"
                if self.cache_dir is not None :
                    challenge_dataset = load_dataset(dataset_path, "ARC-Easy", cache_dir=self.cache_dir)
                else :
                    challenge_dataset = load_dataset(dataset_path, "ARC-Easy")
                dataset = challenge_dataset["test"]
