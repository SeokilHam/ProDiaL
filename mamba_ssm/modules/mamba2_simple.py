...
class Mamba2Simple(nn.Module):
    def __init__(
        ...
        r1 = 768 # proj 64 in_proj 32 out_proj 128
        r2 = 1536
        # import pdb; pdb.set_trace()
        self.in_oft_r = nn.Parameter(torch.zeros(math.ceil(self.d_model / r1), math.ceil(self.d_model / r1)).unsqueeze(0).repeat(r1, 1, 1)) # 768
        self.in_oft_s = nn.Parameter(torch.ones(self.d_inner * 2)) # 3072
...
def DiaL_forward(LoRAModel, DiaL, scale, x, *args, **kwargs):
    ...
    for active_adapter in LoRAModel.active_adapters:
        if active_adapter not in LoRAModel.lora_A.keys():
            continue
        lora_A = LoRAModel.lora_A[active_adapter]
        lora_B = LoRAModel.lora_B[active_adapter]
        dropout = LoRAModel.lora_dropout[active_adapter]
        scaling = LoRAModel.scaling[active_adapter]
